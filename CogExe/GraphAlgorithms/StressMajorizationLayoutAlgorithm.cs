...
namespace SIL.Cog.GraphAlgorithms
{
		...
		where TGraph : IBidirectionalGraph<TVertex, TEdge>
	{
		...
		private void GetDistancesAndWeights(out double[,] d, out double[,] w)
		{
			...
			{
				var spa = new UndirectedDijkstraShortestPathAlgorithm<TVertex, TEdge>(undirected, edge => edge is WeightedEdge<TVertex> ? Parameters.WeightAdjustment - ((edge as WeightedEdge<TVertex>).Weight) : 1.0);
				spa.Compute(source);
				int j = 0;
