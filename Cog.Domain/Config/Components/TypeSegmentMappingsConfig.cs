...
namespace SIL.Cog.Domain.Config.Components
{
	public class TypeSegmentMappingsConfig : IComponentConfig<ISegmentMappings>
	{
		...
		{
			var vowelMappings = ConfigManager.LoadComponent<ISegmentMappings>(segmentPool, project, elem.Element(ConfigManager.Cog + "VowelMappings"));
			var consonantMappings = ConfigManager.LoadComponent<ISegmentMappings>(segmentPool, project, elem.Element(ConfigManager.Cog + "ConsonantMappings"));
